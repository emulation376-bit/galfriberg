...
import { GuessFeedback, GameTitle } from '../types';
...
function hiddenGuess(feedback: GuessFeedback) {
  const hideAttribute = ({ level, hint }: GuessFeedback['attributes']['company']) => ({
    level,
    ...(hint ? { hint } : {}),
  });
  return {
    ...
    attributes: {
      releaseYear: hideAttribute(feedback.attributes.releaseYear),
      company: hideAttribute(feedback.attributes.company),
      isR18: hideAttribute(feedback.attributes.isR18),
      scenarioWriter: hideAttribute(feedback.attributes.scenarioWriter),
      musicComposer: hideAttribute(feedback.attributes.musicComposer),
      artist: hideAttribute(feedback.attributes.artist),
      voiceActor: hideAttribute(feedback.attributes.voiceActor),
      tags: hideAttribute(feedback.attributes.tags),
      bgmScore: hideAttribute(feedback.attributes.bgmScore),
      isSeries: hideAttribute(feedback.attributes.isSeries),
      length: hideAttribute(feedback.attributes.length),
    },
  };
}
...
function replayAnswer(target: GameTitle) {
  return {
    ...
    scenarioWriter: target.scenario_writer,
    musicComposer: target.music_composer,
    artist: target.artist,
    voiceActor: target.voice_actor,
    bgmScore: Number(target.bgm_score),
...
function replayGuesses(target: GameTitle, gameIds: number[]) {
  return gameIds.slice(0, MAX_GUESSES).flatMap((gameId) => {
    const guess = getGame(gameId);
...
function buildMatchReplay(room: StoredRoom, viewerKey: string) {
  ...
  return {
    ...
    rounds: room.replayRounds.flatMap((round) => {
      const target = getGame(round.targetGameId);
      if (!target) return [];
      return [{
        round: round.round,
        reason: round.reason,
...
function answerView(targetGameId: number | null) {
  const target = targetGameId ? getGame(targetGameId) : null;
  return target ? replayAnswer(target) : null;
}
...
function appendReplayRound(room: StoredRoom): void {
  ...
  if (!result || !room.targetGameId) return;
  ...
  room.replayRounds.push({
    ...
    guessesByPlayer: Object.fromEntries(
      room.players.map((player) => [player.key, player.guesses.map((guess) => guess.gameId)])
    ),
  });
...
async function startRound(io: Server, roomId: string) {
  const result = await withRoomLock(roomId, (room) => {
    ...
    if (!target) return { error: 'EMPTY_GAME_POOL' as const };
    room.status = 'playing';
    room.readyCheckEndsAt = null;
    room.round += 1;
...
export function setupSocket(io: Server) {
  ...
  io.on('connection', (socket) => {
    ...
    safeOn(socket, 'game:start', async (_payload, ack) => {
      ...
      ack?.(started ? { ok: true } : { code: 'EMPTY_GAME_POOL' });
    });
    ...
    safeOn(socket, 'game:guess', async (payload: z.infer<typeof gameGuessPayloadSchema>, ack) => {
      ...
      socket.data.roomId = roomId;
      const guess = getEnabledGame(payload.gameId);
      if (!guess) return ack?.({ code: 'TITLE_NOT_FOUND' });
