...
import { initGameCache, getGame } from '../services/playerCache';
...
describe('stats and replay', () => {
  ...
  it('returns personal and global stats and protects replay ownership', async () => {
    ...
    const gameRows = await db('game_titles').select('id').limit(2);
    const target = getGame(Number(gameRows[0].id))!;
    const otherGame = getGame(Number(gameRows[1].id))!;
    ...
    try {
      ...
      // answerView 返回 displayName（优先中文名）
      expect(replay.data.answer.title).toBe(target.title_cn || target.title);
      ...
      expect(multiReplay.data.rounds[0].opponent.guesses[0].gameId).toBe(otherGame.id);
  ...
  it('counts current and legacy first guesses and excludes invalid player ids', async () => {
    ...
    const gameRows = await db('game_titles').select('id').orderBy('id').limit(2);
    const favorite = getGame(Number(gameRows[0].id))!;
    const other = getGame(Number(gameRows[1].id))!;
    const games: Array<{ suffix: string; guesses: unknown[]; firstGuessGameId: number | null }> = [
      { suffix: 'current', guesses: [favorite.id], firstGuessGameId: favorite.id },
      { suffix: 'legacy', guesses: [{ gameId: favorite.id }], firstGuessGameId: null },
      { suffix: 'other', guesses: [other.id], firstGuessGameId: other.id },
      { suffix: 'invalid', guesses: [99999999], firstGuessGameId: 0 },
    ];

    await db('games').insert(games.map((game) => ({
      session_id: `first-guess-${game.suffix}-${stamp}`,
      guest_key: ownerKey,
      target_game_id: favorite.id,
      mode: 'easy',
      guesses: JSON.stringify(game.guesses),
      first_guess_game_id: game.firstGuessGameId,
      status: 'won',
      guess_count: 1,
      finished_at: db.fn.now(),
    })));
