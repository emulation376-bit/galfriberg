...
import {
  allGlobalCharacterStatsCacheKeys,
  allGlobalStatsCacheKeys,
  allPersonalCharacterStatsCacheKeys,
  allPersonalStatsCacheKeys,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const characterClaimed = await db('character_games')
      .where({ guest_key: guestKey })
      .whereNull('user_id')
      .update({ user_id: req.user!.id, guest_key: null });
    ...
    await invalidateCached(
      ...
      ...allPersonalCharacterStatsCacheKeys(`g:${guestKey}`),
      ...allPersonalCharacterStatsCacheKeys(`u:${req.user!.id}`),
      `room-player-performance:g:${guestKey}`,
      `room-player-performance:u:${req.user!.id}`
    );
    res.json({ claimed, characterClaimed });
  })
);
