...
import { gameImportSchema } from '../services/playerMutations';
...
describe('admin user management', () => {
  ...
  it('lists registered users and returns protected performance details', async () => {
    ...
    const [targetGame] = await db('game_titles').select('id').limit(1);
    try {
      await db('games').insert([
        {
          ...
          mode: 'easy',
          guesses: JSON.stringify([targetGame.id]),
          first_guess_game_id: targetGame.id,
          status: 'won',
          guess_count: 2,
        ...
        const [inserted] = await db('match_records')
          .insert({
            ...
            replay: JSON.stringify([{
              ...
              reason: 'guessed',
              guessesByPlayer: {
                [`u:${targetUser.id}`]: [targetGame.id],
                [`g:${opponentKey}`]: [targetGame.id],
              },
            }]),
          })
          .returning('id');
      ...
      expect(multiReplay.data.rounds[0]).toMatchObject({
        answer: { id: targetGame.id },
        me: { guesses: [{ gameId: targetGame.id }] },
        opponent: { guesses: [{ gameId: targetGame.id }] },
      });
    ...
    it('exports all game fields in the JSON import format for admins only', async () => {
    const stamp = Date.now();
    const adminUsername = `admin-export-admin-${stamp}`;
    const userUsername = `admin-export-user-${stamp}`;
    const title = `export-game-${stamp}`;
    const insertedUsers = await db('users')
      .insert([
        {
          username: adminUsername,
    ...
    const [insertedGame] = await db('game_titles')
      .insert({
        title,
        title_cn: '测试导出',
        release_year: 2003,
      ...
      .returning('id');
    const gameId = Number(typeof insertedGame === 'object' ? insertedGame.id : insertedGame);
    try {
      await db('game_difficulties').insert([
        { game_id: gameId, difficulty_key: 'normal' },
        { game_id: gameId, difficulty_key: 'easy' },
      ]);

      const exported = await request('/api/admin/games/export', authCookie(admin));
      expect(exported.response.status).toBe(200);
      expect(exported.response.headers.get('content-disposition')).toContain('games.json');
      expect(() => gameImportSchema.parse({ games: exported.data })).not.toThrow();
      expect(exported.data.find((game: { title: string }) => game.title === title)).toEqual({
        title,
        title_cn: '测试导出',
        release_year: 2003,
        company: 'Export Test',
      ...
      const forbidden = await request('/api/admin/games/export', authCookie(user));
      expect(forbidden.response.status).toBe(403);
      expect(forbidden.data).toEqual({ code: 'FORBIDDEN' });
    } finally {
      await db('game_difficulties').where({ game_id: gameId }).del();
      await db('game_titles').where({ id: gameId }).del();
