...
async function loadPlayerPerformance(identity: StoredIdentity) {
  ...
  const characterQuery = db('character_games').whereNot('status', 'playing');
  if (identity.userId !== null) {
    singleQuery.where({ user_id: identity.userId });
    characterQuery.where({ user_id: identity.userId });
  } else if (identity.key.startsWith('g:')) {
    singleQuery.where({ guest_key: identity.key.slice(2) });
    characterQuery.where({ guest_key: identity.key.slice(2) });
  } else {
    singleQuery.whereRaw('1 = 0');
    characterQuery.whereRaw('1 = 0');
  }

  const [single, character, multi, recentRows] = await Promise.all([
    singleQuery
      .first()
      .count({ games: 'id' })
  ...
  return {
    ...
    character: summary(character, true),
    multi: {
      ...summary(multi, false),
