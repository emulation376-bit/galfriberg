...
// In-memory fallback for PoW challenges when Redis is unavailable
const memoryChallenges = new Map<string, { data: StoredChallenge; expiresAt: number }>();
...
export async function createChallenge(userAgent: string | undefined) {
  ...
  if (!isRedisAvailable()) {
    const expiresAt = Date.now() + config.powChallengeTtlSeconds * 1000;
    memoryChallenges.set(id, { data: stored, expiresAt });
    return {
      id,
      challenge,
...
export async function consumeAndVerifyChallenge(
  ...
  if (!isRedisAvailable()) {
    ...
    if (!entry) throw new PowVerificationError('POW_CHALLENGE_EXPIRED');
