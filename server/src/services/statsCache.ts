...
export function globalCharacterStatsCacheKey(difficulties: readonly string[]): string {
  return `stats:character-global:v1:${selectionKey(difficulties)}`;
}

export function personalCharacterStatsCacheKey(
  identityKey: string,
  difficulties: readonly string[]
): string {
  return `stats:character-personal:v1:${identityKey}:${selectionKey(difficulties)}`;
}

export function allGlobalCharacterStatsCacheKeys(): string[] {
  return allSelections().map(globalCharacterStatsCacheKey);
}

export function allPersonalCharacterStatsCacheKeys(identityKey: string): string[] {
  return allSelections().map((difficulties) =>
    personalCharacterStatsCacheKey(identityKey, difficulties)
  );
}

export function globalCharacterStatsCacheKeysForDifficulty(difficulty: string): string[] {
  return allSelections()
    .filter((selection) => selection.includes(difficulty))
    .map(globalCharacterStatsCacheKey);
}

export function personalCharacterStatsCacheKeysForDifficulty(
  identityKey: string,
  difficulty: string
): string[] {
  return allSelections()
    .filter((selection) => selection.includes(difficulty))
    .map((difficulties) => personalCharacterStatsCacheKey(identityKey, difficulties));
}
