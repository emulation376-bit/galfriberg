...
import characterImageRoutes from './routes/characterImages';
import externalGameRoutes, { externalGameAuth } from './routes/externalPlayers';
...
/**
 * 判断请求来源(Origin)是否允许:仅放行 config.corsOrigins 白名单。
 * 无 Origin 的同源请求(直接导航、curl 等)不受跨域限制。
 */
...
async function main() {
  ...
  app.use('/img', characterImageRoutes);
  ...
  app.use('/api/admin/games/import', requireAuth, requireAdmin);
  app.use(
