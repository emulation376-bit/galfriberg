...
```text
ghcr.io/emulation376-bit/galfriberg
```

运行镜像基于 distroless Node.js，仅包含生产 Node 依赖、编译后的服务端
JavaScript、编译后的前端资源以及输出到 `server/dist` 的作品种子文件。镜像中
...
把整个项目源码（含 `package.json`、`pnpm-lock.yaml`、`pnpm-workspace.yaml`、
`client/`、`server/`、`scripts/`、`pow-wasm/`）上传到服务器后，在部署目录执行：

```bash
docker compose build
```

构建需要能访问 `gcr.io/distroless/nodejs22-debian12:nonroot`。国内服务器拉不到
时可换镜像源：
...
`IMAGE` 变量控制使用的镜像，默认 `galfriberg:latest`（本地构建产物）。若改为
从 GHCR 拉取，把 `IMAGE` 设为 `ghcr.io/emulation376-bit/galfriberg:latest`。
...
首次启动前先构建或拉取镜像（按 [部署方式](#部署方式) 二选一）：

```bash
docker compose build            # 本地构建（默认方式）
# 或
docker compose pull             # 从 GHCR 拉取（需先设置 IMAGE 为 ghcr.io 地址）
```
...
```bash
docker compose build migrate app-1 app-2   # 本地构建；若用 GHCR 则改为 pull
...
```text
IMAGE=ghcr.io/emulation376-bit/galfriberg:sha-0123456
```

回滚时把 `IMAGE` 改回上一个标签并执行：

```bash
docker compose pull migrate app-1 app-2    # 本地构建时改为 build
docker compose up migrate
docker compose up -d --no-deps app-1
docker compose up -d --no-deps app-2
```
...
卷中，更换应用镜像不会影响二者。角色立绘缓存默认写入容器内的
`/tmp/image-cache`（tmpfs），重启后自动清空，可通过 `IMAGE_CACHE_DIR` 调整。
