...
describe('difficulty helpers', () => {
  ...
  it('resolves localized labels and descriptions', () => {
    ...
    expect(difficultyLabel(t, 'beginner')).toBe('热门版');
    expect(difficultyDescription(t, 'beginner')).toBe('高知名度作品池 · 熟悉规则');
    expect(difficultyLabel(t, 'easy')).toBe('进阶版');
    expect(difficultyDescription(t, 'easy')).toBe('知名作品池 · 快速上手');
