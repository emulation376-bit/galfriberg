...
describe('desktop/mobile layout contracts', () => {
  ...
  it('caps single difficulty cards on wide screens and stacks actions on mobile', () => {
    ...
    expect(home).toMatch(
      /\.single-difficulty-grid\s*\{[^}]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/s
    );
    expect(home).toMatch(/\.single-difficulty-grid\s*\{[^}]*margin-inline:\s*auto/s);
    expect(home).toMatch(
    ...
    expect(responsive).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.single-difficulty-grid\s*\{[^}]*grid-template-columns:\s*1fr/
    );
  ...
  it('scales the multiplayer board continuously so browser zoom cannot snap its density', () => {
    ...
    // 11 列: 名称/年份/会社/限制级/系列作/时长/BGM/标签/剧本/原画/声优
    const demand = [
      { w: 21, n: 11, cjk: false }, { w: 9, n: 4, cjk: false },
      { w: 14, n: 7, cjk: false }, { w: 9, n: 2, cjk: false, arrow: true },
      { w: 6, n: 1, cjk: true }, { w: 12, n: 2, cjk: true, arrow: true },
      { w: 8, n: 1, cjk: false, arrow: true }, { w: 9, n: 2, cjk: true },
      { w: 13, n: 3, cjk: true }, { w: 10, n: 2, cjk: true },
      { w: 10, n: 2, cjk: true },
    ];
