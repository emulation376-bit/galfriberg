...
import { BarChart3, Check, ChevronDown, ChevronLeft, ChevronRight, Gamepad2, Play, Swords, User, Users } from 'lucide-react';
...
import { AVAILABLE_DIFFICULTIES, STAT_ELIGIBLE_DIFFICULTIES } from '../config/difficulties';
...
interface SingleStats {
  ...
  firstGuess: { gameId: number; title: string; percentage: number } | null;
}
...
interface CharacterStatsResponse {
  difficulties: string[];
  personal: SingleStats & { firstGuess: CharacterFirstGuess | null };
  global: SingleStats & {
    firstGuess: CharacterFirstGuess | null;
    registeredUsers: number;
  };
}
...
type ReplayType = 'single' | 'multi' | 'character';
type StatsDimension = 'game' | 'character';

const STATS_DIFFICULTIES = STAT_ELIGIBLE_DIFFICULTIES.map((difficulty) => difficulty.key);

function formatAverage(value: number | null): string {
  return value == null ? '-' : value.toFixed(2);
}

function formatFirstGuess(value: SingleStats['firstGuess'] | CharacterFirstGuess | null): string {
  if (!value) return '-';
  if ('characterId' in value) {
    return `${value.name} ${(value.percentage * 100).toFixed(1)}%`;
  }
  return `${value.title} ${(value.percentage * 100).toFixed(1)}%`;
}
...
function StatsDimensionTabs({
  value,
  onChange,
}: {
  value: StatsDimension;
  onChange: (next: StatsDimension) => void;
}) {
  const { t } = useTranslation();
  return (
    <div className="stats-replay-segments stats-dimension-tabs" role="tablist" aria-label={t('stats.dimensionLabel')}>
      <button
        type="button"
        role="tab"
        aria-selected={value === 'game'}
        className={value === 'game' ? 'active' : ''}
        onClick={() => onChange('game')}
      >
        <Gamepad2 size={15} />{t('stats.game')}
      </button>
      <button
        type="button"
        role="tab"
      ...
      >
        <User size={15} />{t('stats.character')}
      </button>
    </div>
  );
}
...
export default function Stats() {
  ...
  const [dimension, setDimension] = useState<StatsDimension>('game');
  const [stats, setStats] = useState<StatsResponse | CharacterStatsResponse | null>(null);
  const [type, setType] = useState<ReplayType>('single');
  const [page, setPage] = useState(1);
  const [items, setItems] = useState<Array<SingleReplayItem | MultiReplayItem | CharacterReplayItem>>([]);
  ...
  const loadStats = useCallback(() => {
    ...
    setStatsError(false);
    const url = dimension === 'character' ? '/stats/character' : '/stats/me';
    api.get<StatsResponse | CharacterStatsResponse>(url, {
  ...
  }, [dimension, selectedDifficulties]);
  ...
  const loadReplays = useCallback(async () => {
    ...
    try {
      const res = type === 'character'
        ? await api.get<ReplayPage<CharacterReplayItem>>('/stats/character/replays', {
          params: { page, pageSize: 15 },
        })
        : await api.get<ReplayPage<SingleReplayItem | MultiReplayItem>>('/stats/replays', {
  ...
  const chooseDimension = (next: StatsDimension) => {
    setDimension(next);
    setStats(null);
  };

  const openReplay = async (item: SingleReplayItem | MultiReplayItem | CharacterReplayItem) => {
    ...
    try {
      if (item.type === 'single') {
        ...
        const res = await api.get<Omit<CharacterSingleReplay, 'type'>>(
          `/stats/character/games/${item.id}/replay`
        );
        setReplay({ type: 'character', ...res.data });
      }
    } catch (err) {
  ...
  const replayButton = (item: SingleReplayItem | MultiReplayItem | CharacterReplayItem) => (
    <button
  ...
  const characterColumns: Column<CharacterReplayItem>[] = [
    { key: 'mode', title: t('stats.mode'), render: (game) => difficultyLabel(t, game.mode) },
    { key: 'status', title: t('stats.result'), render: (game) => game.status === 'won'
      ? <Badge text={t('common.win')} color="green" /> : <Badge text={t('common.loss')} color="gray" /> },
    { key: 'guessCount', title: t('stats.guesses') },
        ...
        {!stats && statsLoading && (
          <section className="stats-difficulty-section" aria-busy="true">
            <div className="stats-difficulty-toolbar">
              <StatsDimensionTabs value={dimension} onChange={chooseDimension} />
          ...
          <section className="stats-difficulty-section" aria-busy={statsLoading}>
            ...
            <div className="stats-overview-grid">
              <div className="card">
                ...
                <StatTable rows={[
                  [t(dimension === 'character' ? 'stats.characterGames' : 'stats.singleGames'), stats.personal.totalGames],
                  [t(dimension === 'character' ? 'stats.characterWins' : 'stats.singleWins'), stats.personal.wins],
                  ...
                  ...(dimension === 'game' ? [
                    [t('stats.multiGamesWins'), `${(stats as StatsResponse).personal.multiGames} / ${(stats as StatsResponse).personal.multiWins}`],
                    [t('stats.multiAvgWinningGuesses'), formatAverage((stats as StatsResponse).personal.multiAvgWinningGuesses)],
              ...
              <div className="card">
                ...
                <StatTable rows={[
                  ...
                  [t(dimension === 'character' ? 'stats.characterGames' : 'stats.singleGames'), stats.global.totalGames],
                  [t(dimension === 'character' ? 'stats.characterWins' : 'stats.singleWins'), stats.global.wins],
                  ...
                  ...(dimension === 'game' ? [
                    [t('stats.multiGames'), (stats as StatsResponse).global.multiGames],
                    [t('stats.multiAvgWinningGuesses'), formatAverage((stats as StatsResponse).global.multiAvgWinningGuesses)],
                  ] as [string, string | number][] : []),
                ]} />
              </div>
            </div>
        ...
        <section className="card stats-recent-card">
          <div className="stats-replay-toolbar">
            ...
            <div className="stats-replay-segments" role="tablist" aria-label={t('stats.replayType')}>
              ...
              <button type="button" role="tab" aria-selected={type === 'character'} className={type === 'character' ? 'active' : ''} onClick={() => chooseType('character')}>
                <User size={15} />{t('stats.character')}
              </button>
            </div>
          </div>
          <div className="stats-recent-table stats-replay-desktop-list">
            {type === 'single' ? (
              ...
              <DataTable
                ...
                rows={items.filter((item): item is CharacterReplayItem => item.type === 'character')}
                rowKey={(game) => game.id}
                loading={loading}
                empty={t('stats.noCharacter')}
              />
            ) : (
              <DataTable
            ...
            {items.length ? items.map((item) => {
              ...
              <article className="stats-replay-mobile-item" key={`${item.type}:${item.id}`}>
                ...
                <div className="stats-replay-mobile-details">
                  {item.type === 'single' || item.type === 'character' ? (
                    <>
