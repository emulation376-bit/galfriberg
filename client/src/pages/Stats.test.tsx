...
describe('Stats difficulty filter', () => {
  beforeEach(() => {
    ...
    apiGet.mockImplementation((url: string, config?: { params?: { difficulties?: string } }) => {
        ...
        return Promise.resolve({
          data: {
            ...
            personal: {
              ...
              avgGuesses: 3,
              bestGuesses: 2,
              firstGuess: { characterId: 'c1', name: '茜崎空', percentage: 0.5 },
            },
            global: {
              totalGames: 20,
              wins: 10,
      ...
      if (url === '/stats/character/replays') {
        return Promise.resolve({
          data: { type: 'character', page: 1, pageSize: 15, hasNext: false, items: [] },
        });
      }
  ...
  it('merges selected difficulties into one personal and global summary', async () => {
    ...
    await user.click(within(difficultyGroup).getByRole('checkbox', { name: '热门版' }));

    await waitFor(() => expect(apiGet).toHaveBeenCalledWith('/stats/me', {
      params: { difficulties: 'easy,normal' },
    }));
    expect(screen.getByText('进阶版, 完整版', { selector: '.difficulty-multi-select-summary' })).toBeInTheDocument();
  ...
  it('switches to the character dimension and character replays', async () => {
    const user = userEvent.setup();
    renderAtRoute(<Stats />);
    ...
    await user.click(screen.getAllByRole('tab', { name: '猜角色' })[0]);
    await waitFor(() => expect(apiGet).toHaveBeenCalledWith('/stats/character', {
      params: { difficulties: 'beginner,easy,normal' },
    }));
    expect((await screen.findAllByText('猜角色总场次')).length).toBe(2);
    const personalCard = screen.getByRole('heading', { name: '个人统计', level: 3 }).closest('.card') as HTMLElement;
    expect(within(personalCard).getByText('猜角色总场次').closest('tr')).toHaveTextContent('2');

    await user.click(screen.getAllByRole('tab', { name: '猜角色' })[1]);
    await waitFor(() => expect(apiGet).toHaveBeenCalledWith('/stats/character/replays', {
