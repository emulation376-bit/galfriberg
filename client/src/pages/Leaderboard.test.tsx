...
describe('Leaderboard filters', () => {
  ...
  it('switches to the character leaderboard tab', async () => {
    const user = userEvent.setup();
    renderAtRoute(<Leaderboard />);

    await user.click(screen.getByRole('tab', { name: '猜角色' }));
    await waitFor(() => expect(apiGet).toHaveBeenLastCalledWith('/leaderboard', {
      params: { mode: 'character', difficulty: 'beginner' },
    }));
    expect(screen.getByRole('tab', { name: '猜角色' })).toHaveAttribute('aria-selected', 'true');
  });
});
