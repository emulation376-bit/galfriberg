...
export default function SingleGame() {
  ...
  }, [isValidMode, mode, navigate, customPoolMissing]);
  ...
  const guess = async (guessGameId: number) => {
    if (!gameId || status !== 'playing' || busy) return false;
    try {
      const res = await api.post(`/game/${gameId}/guess`, { gameId: guessGameId });
