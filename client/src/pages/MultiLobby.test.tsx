...
import MultiLobby, { roomInviteUrl } from './MultiLobby';
...
describe('MultiLobby matchmaking', () => {
  beforeEach(() => {
    ...
    socket.emit.mockImplementation((event: string, ...args: any[]) => {
      ...
      if (event === 'room:create' && typeof ack === 'function') ack({
        room: {
          id: 'ABC12',
          dbType: 'normal',
          boType: 3,
      ...
      if (event === 'room:join' && typeof ack === 'function') ack({});
    });
  });
  ...
  it('remembers create-room and matchmaking options on this browser', async () => {
    ...
    const easyButtons = await screen.findAllByRole('button', { name: '进阶版' });
    ...
    const restoredEasyButtons = await screen.findAllByRole('button', { name: '进阶版' });
  ...
  it('copies the direct room invite link to the clipboard', async () => {
    const user = userEvent.setup();
    ...
    await user.click(await screen.findByRole('button', { name: '复制邀请链接' }));

    await waitFor(() => {
      expect(writeText).toHaveBeenCalledWith(roomInviteUrl('ABC12'));
    });
  });

  it('joins directly from an invite link query', async () => {
    renderAtRoute(<MultiLobby />, {
      route: '/multi?join=ABC12',
      path: '/multi',
      extraRoutes: <Route path="/multi/room" element={<div>joined room</div>} />,
    });

    await waitFor(() => {
      expect(socket.emit).toHaveBeenCalledWith(
        'room:join',
        { roomId: 'ABC12', spectate: false },
        expect.any(Function)
      );
    });
    expect(await screen.findByText('joined room')).toBeInTheDocument();
  });
});
