...
vi.mock('../api/playerList', () => ({
  getGameList: vi.fn(async () => [{ id: 1, title: 'CLANNAD' }]),
  subscribeGameList: vi.fn(() => () => undefined),
  searchGameList: (list: Array<{ id: number; title: string }>, query: string) =>
...
async function waitForReadyInput() {
  const input = await screen.findByPlaceholderText('输入作品名称...');
...
describe('SingleGame UX', () => {
  ...
  it('shows starting feedback and keeps dock input disabled while start is pending', async () => {
    ...
    expect(screen.getByText('在下方输入作品名称开始猜测')).toBeInTheDocument();
  ...
  it('shows reveal busy state on the action button and top status bar', async () => {
    ...
    const giveup = deferred<{ data: { answer: { title: string; company: string } } }>();
    ...
    giveup.resolve({
      data: {
        answer: { title: 'CLANNAD', company: 'Key' },
      },
    });
    expect(await screen.findByRole('dialog')).toHaveTextContent('CLANNAD');
  });
  ...
  it('keeps an unrecorded settlement warning visible in the answer dialog', async () => {
    ...
    expect(result).toHaveTextContent('CLANNAD');
  ...
  it('confirms returning to the difficulty lobby with lobby-specific copy', async () => {
    post.mockResolvedValueOnce({ data: { gameId: 'g1', guesses: [], maxGuesses: 8 } } as never);
    renderGame('easy');
    await waitForReadyInput();
    ...
    const dialog = await screen.findByRole('alertdialog');
    expect(dialog).toHaveTextContent('返回难度选择？');
    expect(within(dialog).getByRole('button', { name: '返回难度选择' })).toBeInTheDocument();
    expect(dialog).not.toHaveTextContent('返回主菜单');
  });
});
