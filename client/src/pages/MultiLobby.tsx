...
export default function MultiLobby() {
  ...
  useEffect(() => {
    ...
    navigate('/multi', { replace: true, state: location.state ?? null });
    void join(code);
  }, [join, location.search, location.state, navigate]);
  ...
  const copyCode = async () => {
    ...
    try {
      await navigator.clipboard.writeText(roomInviteUrl(createdRoom.id));
      setCopied(true);
