...
import CharacterCustomFilterDialog from '../components/CharacterCustomFilterDialog';
import CharacterGameRules from '../components/CharacterGameRules';
...
export default function CharacterLobby() {
  ...
  const start = () => {
    ...
    if (selectedDifficulty.key === 'custom') {
      setShowCustomFilter(true);
      return;
    }
    navigate(`/character/${selectedDifficulty.key}`);
  };

  return (
    <Page title={t('character.lobbyTitle')} icon={<User size={17} />}>
      <p className="muted single-lobby-subtitle">{t('character.lobbySubtitle')}</p>
      {difficulties.length ? (
        <>
              ...
              return (
                ...
                >
                  ...
                  <span className="single-difficulty-copy">
                    ...
                    <small>{difficultyDescription(t, difficulty.key) || t('singleLobby.available')}</small>
                  </span>
      ...
      <CharacterCustomFilterDialog open={showCustomFilter} onClose={() => setShowCustomFilter(false)} />
    </Page>
  );
}
