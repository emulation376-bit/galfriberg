...
type LeaderboardMode = 'single' | 'multi' | 'character';
...
export default function Leaderboard() {
  ...
  const difficulties = STAT_ELIGIBLE_DIFFICULTIES;
  const [mode, setMode] = useState<LeaderboardMode>('single');
  const [difficulty, setDifficulty] = useState(STAT_ELIGIBLE_DIFFICULTIES[0]?.key ?? 'beginner');
    ...
    <Page title={t('leaderboard.title')} icon={<Trophy size={17} />}>
      ...
      <div className="leaderboard-controls">
        <div className="leaderboard-mode-tabs" role="tablist" aria-label={t('leaderboard.modeLabel')}>
          {(['single', 'multi', 'character'] as const).map((option) => (
            <button
              type="button"
              role="tab"
