...
type UserGame = SingleUserGame | MultiUserGame | CharacterUserGame;
...
function UserStatsDialog({ view, onClose }: { view: UserStatsView; onClose: () => void }) {
  ...
  const { single, character, multi } = view.stats;
  return (
    <ModalPortal>
      <div className="admin-player-backdrop" onMouseDown={(event) => {
        ...
        <div className="admin-player-dialog admin-user-stats-dialog" role="dialog" aria-modal="true" aria-labelledby="admin-user-stats-title">
          ...
          <div className="admin-user-stats-grid">
            ...
            <section>
              <h3>{t('multi.characterStats')}</h3>
              <dl className="player-stats-list">
                <div><dt>{t('multi.games')}</dt><dd>{character.games}</dd></div>
                <div><dt>{t('multi.winsLosses')}</dt><dd>{character.wins} / {character.losses}</dd></div>
                <div><dt>{t('multi.winRate')}</dt><dd>{formatWinRate(character.winRate)}</dd></div>
                <div><dt>{t('multi.avgWinningGuesses')}</dt><dd>{character.avgGuesses?.toFixed(1) ?? '-'}</dd></div>
                <div><dt>{t('multi.fastest')}</dt><dd>{character.bestGuesses ?? '-'}</dd></div>
              </dl>
            </section>
            <section>
...
function UserGamesDialog({ user, onClose }: { user: AdminUser; onClose: () => void }) {
  const { t } = useTranslation();
  const [type, setType] = useState<'single' | 'multi' | 'character'>('single');
  ...
  const chooseType = (next: 'single' | 'multi' | 'character') => {
    setType(next);
    setPage(1);
    setItems([]);
  };
    ...
    try {
      if (game.type === 'single') {
        ...
        const res = await api.get<Omit<CharacterSingleReplay, 'type'>>(
          `/admin/users/${user.id}/character-games/${game.id}/replay`
        );
        ...
        <div className="admin-player-backdrop" onMouseDown={(event) => {
          ...
          <div className="stats-replay-segments admin-user-game-tabs" role="tablist" aria-label={t('admin.gameType')}>
            ...
            <button type="button" role="tab" aria-selected={type === 'character'} className={type === 'character' ? 'active' : ''} onClick={() => chooseType('character')}>
              <User size={15} />{t('admin.character')}
            </button>
          </div>
          <div className="admin-user-game-list">
            {items.length ? items.map((game) => {
              const result = game.type === 'single' || game.type === 'character'
                ? game.status
                : game.result;
              ...
              return (
                <article className="admin-user-game-item" key={`${game.type}:${game.id}`}>
                  <div className="admin-user-game-heading">
                    <strong>{game.type === 'single' || game.type === 'character'
                  ...
                  <div className="admin-user-game-details">
                    {game.type === 'single' || game.type === 'character' ? (
                      <>
