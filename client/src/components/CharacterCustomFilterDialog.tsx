...
export default function CharacterCustomFilterDialog({ open, onClose }: Props) {
  ...
  const applyFilter = async (next: CustomFilterCriteria, save = true) => {
    setChecking(true);
    try {
      const res = await api.post<FilterResult>('/characters/filter', {
  ...
  const start = async () => {
    ...
    try {
      ...
      onClose();
      navigate('/character/custom', { state: { pool: res.data.poolKey } });
    } catch (err) {
      toast.error(errMsg(err));
    } finally {
      setBusy(false);
    ...
    <ModalPortal>
      <div className="confirm-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
        <div className="confirm-dialog custom-filter-dialog" role="dialog" aria-modal="true" aria-label={t('customFilter.title')}>
          ...
          <p className="muted custom-filter-subtitle">{t('character.customFilterSubtitle')}</p>
          <div className="custom-filter-grid">
            ...
            {field(t('customFilter.maxGuesses'), 'maxGuesses', '8', t('customFilter.maxGuessesHint'))}
          </div>
