...
import { getGameList, searchGameList, subscribeGameList } from '../api/playerList';
...
interface Props {
  ...
  buttonText?: string;
  options?: Suggestion[];
  searchOptions?: (list: Suggestion[], query: string) => Suggestion[];
}
...
export default function GuessInputBar({
  ...
  const applyQuery = useCallback((
    ...
    const next = searchOptions
      ? searchOptions(list, query)
      : searchGameList(list, query);
    setItems(next);
  ...
  useEffect(() => {
    if (options) {
      players.current = options;
      applyQuery(textRef.current, options, false);
      return;
    }
    const unsubscribe = subscribeGameList(applyPlayerList);
    void getGameList().then(applyPlayerList).catch((error) => toast.error(errMsg(error)));
    return unsubscribe;
  }, [applyPlayerList, applyQuery, options]);
          ...
          {items.map((item, i) => (
            ...
            >
              {item.subtitle ? (
                <span>
                  {item.title}
                  <small className="guess-suggestion-subtitle">{item.subtitle}</small>
                </span>
              ) : item.title}
            </li>
          ))}
      ...
      <form className="input-bar" onSubmit={submit}>
        <input
          ...
          onFocus={() => {
            ...
            if (!options) void getGameList().catch(() => undefined);
            onFocusChange?.(true);
          }}
          onBlur={() => {
          ...
          onKeyDown={(e) => {
            ...
            if (e.key === 'ArrowDown') {
              ...
              if (items.length === 1 && tabCycling.current && text === items[0].title) {
                tabCycling.current = false;
                setOpen(false);
                return;
              }
              ...
              const nextActive = tabCycling.current
                ...
                : e.shiftKey
                  ...
                  : active;
              const completed = items[nextActive].title;
